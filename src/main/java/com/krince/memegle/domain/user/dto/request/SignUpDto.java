...
public class SignUpDto {
    ...
    @Schema(title = "로그인 아이디", description = "회원 로그인 아이디(6 ~ 15 영어 소문자, 숫자 조합)", example = "login1")
    @NotBlank(message = "아이디를 입력해주세요.")
    @Size(min = 6, max = 15, message = "아이디는 6자 이상 15자 이하여야합니다.")
    @Pattern(regexp = "^[a-z0-9]*$", message = "영어 소문자와 숫자만 사용할 수 있습니다.")
    private String loginId;

    @Schema(title = "비밀번호", description = "8 ~ 20자 영어 소문자, 대문자, 숫자 조합", example = "TestPassword1")
