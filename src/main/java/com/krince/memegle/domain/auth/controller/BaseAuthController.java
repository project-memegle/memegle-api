...
public abstract class BaseAuthController {
    ...
    @GetMapping("/email")
    @Operation(summary = "이메일 중복 검증(미구현 api)", description = "해당 이메일이 이미 사용되는 이메일인지 검증합니다. 중복된 이메일이라면 예외를 반환합니다.")
    ...
    public abstract ResponseEntity<ResponseCode> validateDuplicateMail(@RequestParam String email);
